...
% unify coefficients
if isscalar(a), a=a*ones(1,nt); end
if size(a,1)==4 && size(a,2)==1, a=a*ones(1,nt); end
if size(a,1)==1, a=[a;0*a;0*a;a]; end
%
if isscalar(c), c=c*ones(1,nt); end
if isscalar(f), f=f*ones(1,nt); end
%
...
for k=1:nt % loop over elements
    ...
    B=[p2-p1,p3-p1]; iBt=inv(B)'; Ak=abs(det(B))/2;
    %
    % stiffness
    dphih=[-1,1,0; -1,0,1]; dphi=iBt*dphih;
    ah=reshape(a(:,k),2,2);
    elmat=dphi'*(ah*dphi)*Ak;
    elK(:,k)=elmat(:);
    %
    % mass matrix
    elmat=zeros(3,3);
    xi=[0.5,0.5,0]; yi=[0,0.5,0.5]; wi=[1,1,1]/3;
    for l=1:length(wi)
